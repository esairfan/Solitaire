...
from tableau import Tableau  # Import the Tableau class
...
deck = Deck()
tableau = Tableau(deck)  # Create a Tableau instance
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if selected_cards:
                ...
                if tableau.is_valid_move(selected_cards[0][0], target_column):
                    ...
                    if tableau.get_column(selected_column):
                        tableau.get_column(selected_column)[-1][0].revealed = True  # Reveal the new top card
                else:
            ...
            if selected_cards:
                ...
                tableau.render(screen)  # Render the tableau

                # Now render all the dragged cards stacked vertically
                for i, (card, _) in enumerate(selected_cards):
                    # Calculate the position for each card based on its index
                    screen.blit(card[0].image, (mouse_x + offset_x, mouse_y + offset_y + i * 20))  # Adjust vertical spacing
